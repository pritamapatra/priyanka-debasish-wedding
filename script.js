(function () {
    ...
    if (heroParticles) {
       ...
       then drift magnetically into name positions.
       On tap: 3D perspective scatter (TEDx style).
    =========================================== */

    var splashEl    = document.getElementById('splash');
    var splashStage = document.getElementById('splash-stage');

    if (splashEl && splashStage) {
        ...
        var sFontSize  = isMobile
                         ? Math.max(28, Math.floor(sW / 9))
                         : Math.max(50, Math.floor(sW / 13));
        var sWedsSize  = Math.floor(sFontSize * 0.40);
        var sLineGap   = Math.floor(sFontSize * 0.60);

        /* Clamp font so widest name fits within 82% screen width */
        var sTestCvs  = document.createElement('canvas');
        var sTestCtx  = sTestCvs.getContext('2d');
        sTestCtx.font = '300 italic ' + sFontSize
                        + 'px "EB Garamond", Georgia, serif';
        while (sTestCtx.measureText('Priyanka').width > sW * 0.82
               && sFontSize > 18) {
            sFontSize  -= 2;
            sWedsSize   = Math.floor(sFontSize * 0.40);
            sTestCtx.font = '300 italic ' + sFontSize
                            + 'px "EB Garamond", Georgia, serif';
        }
        ...
        function createMeasurementDiv() {
            ...
            sWords.forEach(function (wordObj, wi) {
                var row = document.createElement('div');
                row.style.cssText = [
                    'font-family:"EB Garamond",Georgia,serif',
                    'font-size:'      + wordObj.fontSize   + 'px',
                    'font-weight:'    + wordObj.fontWeight,
                    'font-style:'     + wordObj.fontStyle,
                    'letter-spacing:' + wordObj.letterSpacing,
                    'white-space:nowrap',
                    'display:flex',
                    'line-height:1'
                ].join(';');

                wordObj.text.split('').forEach(function (ch, ci) {
            ...
            return { wrap: wrap, charData: charData };
        }

        function buildLetters(charData) {
            charData.forEach(function (item, i) {
                var r    = item.span.getBoundingClientRect();
                var tgt  = { left: r.left, top: r.top,
                ...
                el.classList.add('splash__letter');

                el.style.fontFamily    = '"EB Garamond", Georgia, serif';
                el.style.fontSize      = tgt.wordObj.fontSize   + 'px';
                el.style.fontWeight    = tgt.wordObj.fontWeight;
                el.style.fontStyle     = tgt.wordObj.fontStyle;

                /* Random scatter position across screen */
                var rX = Math.random() * sW * 0.9 + sW * 0.05;
                var rY = Math.random() * sH * 0.85 + sH * 0.05;

                el.style.transform = 'translate('
                    + rX + 'px, ' + rY + 'px)';
                el.style.opacity   = '0';

                splashStage.appendChild(el);
                sLetters.push({ el: el, tgt: tgt, rX: rX, rY: rY });
            ...
            /* Start magnetic drift after letters visible */
            setTimeout(startDrift, 1800);
        }

        function startDrift() {
            sLetters.forEach(function (item, i) {
                setTimeout(function () {
                    ...
                    item.el.style.transform  = 'translate('
                        + item.tgt.left + 'px, '
                        + item.tgt.top  + 'px)';
                    item.el.style.opacity    = '0.95';
                    item.el.style.textShadow = [
                        '0 0 22px rgba(200,150,30,0.85)',
                        '0 0 44px rgba(200,150,30,0.35)',
                        '0 2px 8px rgba(0,0,0,0.65)'
                    ].join(',');
                }, i * 14);
            });

            /* Show prompt after all letters settle */
            var settleDelay = 1800 + sLetters.length * 14 + 600;
            setTimeout(function () {
        ...
        function triggerScatter() {
            ...
            sLetters.forEach(function (item) {
                /* Letter center in viewport */
                var lx   = item.tgt.left + item.tgt.w / 2;
                var ly   = item.tgt.top  + item.tgt.h / 2;
                var dx   = lx - cx;
                var dy   = ly - cy;
                var dist = Math.sqrt(dx * dx + dy * dy) || 1;

                /* Outward speed — faster for outer letters */
                var speed = Math.random() * 260 + 180;
                var sx    = (dx / dist) * speed;
                var sy    = (dy / dist) * speed;

                /* 3D scale: letters near center zoom toward viewer most,
                   simulating perspective explosion (TEDx effect).
                   distRatio 0 = center (huge scale), 1 = edge (small scale) */
                var distRatio  = dist / maxDist;
                var zoomScale  = (1 - distRatio) * 9 + 1.5;

                var finalX = item.tgt.left + sx;
                var finalY = item.tgt.top  + sy;

                item.el.style.transition = [
                    'transform 0.8s cubic-bezier(0.55, 0, 1, 0.5)',
                    'opacity   0.6s ease'
                ].join(',');
                item.el.style.transform = 'translate('
                    + finalX + 'px, '
                    + finalY + 'px) scale(' + zoomScale + ')';
        ...
        document.fonts.ready.then(function () {
            var meas = createMeasurementDiv();
            document.body.appendChild(meas.wrap);

            requestAnimationFrame(function () {
                requestAnimationFrame(function () {
                    buildLetters(meas.charData);
                    document.body.removeChild(meas.wrap);
                });
            });
        });

        splashEl.addEventListener('click', triggerScatter);
        splashEl.addEventListener('touchstart', triggerScatter,
                                  { passive: true });
        document.addEventListener('keydown', function (e) {
            if (e.key === 'Enter' || e.key === ' ') triggerScatter();
        });
    }

        /* ===========================================
       GALLERY IMAGE FADE-IN
       Adds .is-loaded once each image is decoded.
    =========================================== */
    document.querySelectorAll('.gallery__tile img').forEach(function (img) {
